...
"Perl module Image::Size is required to determine width and height of uploaded images."
    ...
    if ( !$asset ) {
        ...
        # just for easiness
        # we must not skip if timestamp has changed
        return 0;
...
sub remove_orphaned_assets {
    my ($param) = @_;
    my $blog_id = $param->{blog_id};

    require MT::Asset::Image;
    my $iter = MT::Asset::Image->load_iter( { blog_id => $blog_id } );
    while ( my $asset = $iter->() ) {
        unless ( -e $asset->file_path ) {
            $asset->remove or die $asset->errstr;
            print STDERR "File '" . $asset->file_path . "' removed.\n";
        }
    }

    require MT::Asset::Audio;
    my $iter = MT::Asset::Audio->load_iter( { blog_id => $blog_id } );
    while ( my $asset = $iter->() ) {
        unless ( -e $asset->file_path ) {
            $asset->remove or die $asset->errstr;
            print STDERR "File '" . $asset->file_path . "' removed.\n";
    ...
    my $iter = MT::Asset::Video->load_iter( { blog_id => $blog_id } );
    while ( my $asset = $iter->() ) {
        unless ( -e $asset->file_path ) {
            $asset->remove or die $asset->errstr;
            print STDERR "File '" . $asset->file_path . "' removed.\n";
